...
Artificial protein sequences are generated by sampling from the statistical distribution
learned over the training set.  
The generation process respects positional amino-acid frequencies inferred from the OHE
matrix, producing sequences that are statistically plausible but do not appear verbatim
...
├── configurations/                         # Config files (hyperparameters, paths)
│
